...
def tfrec_load(tfrec_file):
    ...
    for string_record in record_iterator:
        ...
        annot_string = (example.features.feature['annotation/encoded'].bytes_list.value[0])
        
        img_1d = np.fromstring(img_string, dtype=np.uint8) # The 1D string to array
...
def batch(dataset,batch_size=3, height=360, width=480, resized=224): # Resize to a multiple of 32
    ...
    raw_annotation = tf.image.resize_nearest_neighbor(raw_annotation, [height, width])
